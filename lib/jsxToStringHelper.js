...
module.exports = function(t) {
  ...
  var JSX_FRAGMENT_PRAGMA = 'JSX.fragment';
  var JSX_FRAGMENT_PRAGMA_REGEX = /<\/?JSX\.fragment>/g;
  ...
  function buildJsxString(path) {
    ...
    if (jsxTemplate.indexOf('{') > -1) {
      jsxTemplate = jsxTemplate.replace(CLEAN_JSX_ATTRIBUTES_VALUES_REGEX, CLEAN_JSX_ATTRIBUTES_VALUES_REPLACE);
    }
    if (jsxTemplate.indexOf(JSX_FRAGMENT_PRAGMA) > -1) {
      jsxTemplate = jsxTemplate.replace(JSX_FRAGMENT_PRAGMA_REGEX, '');
    }
  ...
  function createInterpolatedExpression(interpolatedString) {
    return t.identifier('JSX.toStr(' + interpolatedString + ')');
  }
